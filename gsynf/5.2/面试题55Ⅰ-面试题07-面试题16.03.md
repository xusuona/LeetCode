...
前序遍历为根左右，中序遍历为左根右，按照前序遍历中第一个元素可将中序遍历分开，继续递归即可。
...
class Solution {
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        int index = findInIn(root, inorder);  // 寻找root元素在inorder数组中的索引
        // 左右子树分别递归
    ...
    private int findInIn(int root, int[] inorder) {
        int index = -1;
...
链接：[戳戳戳](https://leetcode-cn.com/problems/intersection-lcci/solution/shi-yong-zhi-xian-xie-jie-shi-fang-cheng-zai-pan-d/)。
